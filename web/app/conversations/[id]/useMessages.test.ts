...
describe('useMessages (Phase 8.4)', () => {
  beforeEach(() => {
  ...
  it('réception temps réel : un message d’un autre participant est ajouté, canal filtré par conversation', async () => {
    ...
    expect(result.current.messages).toEqual([
      { id: 'm1', conversationId: 'c1', senderId: 'other', content: 'Salut', messageType: 'text', createdAt: '2026-01-01T00:00:00Z', status: 'sent' },
    ]);
  });
  ...
  it('déduplication : un message déjà reçu (sent) n’est jamais dupliqué', async () => {
    ...
    const initial = [
      { id: 'm1', conversationId: 'c1', senderId: 'other', content: 'Salut', messageType: 'text' as const, createdAt: '2026-01-01T00:00:00Z' },
    ];
  ...
  it('envoi réussi : message optimiste (pending) puis remplacé par le message serveur (sent)', async () => {
    ...
    await act(async () => {
      resolveSend({
        message: { id: 'm1', conversationId: 'c1', senderId: 'me', content: 'Salut', messageType: 'text', createdAt: '2026-01-01T00:00:00Z' },
      });
      await new Promise((resolveWait) => setTimeout(resolveWait, 0));
    });

    expect(result.current.messages).toEqual([
      { id: 'm1', conversationId: 'c1', senderId: 'me', content: 'Salut', messageType: 'text', createdAt: '2026-01-01T00:00:00Z', status: 'sent' },
    ]);
